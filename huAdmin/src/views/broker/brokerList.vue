...
<template lang="html">
  <div class="brokerList">
    <p class="borTitle">
      券商列表
      <el-button icon="el-icon-plus" type="primary" size="small" @click="addBro=true">添加券商</el-button>
      <el-tooltip class="item" effect="dark" content="刷新数据" placement="bottom">
        <i class="el-icon-refresh" @click="getBroList()"></i>
      </el-tooltip>
    </p>
    <div class="" style="padding:10px;">
      ...
      <noData ref="noData"/>
      <div class="" style="minHeight:600px;" v-loading="broLoading">
  ...
  methods:{
    ...
    delBro(_html){//删除券商
      this.$confirm('此操作将删除当前券商, 是否继续?', '提示', {
          ...
          this.$http.deleteListAPI('securities',{
    ...
    getBroList(){
      ...
      this.$http.getListAPI('securities?page='+this.currentPage3+'&limit=15').then(res=>{
        if(res.data.state==1){
          this.broLoading=false;
          this.broList=res.data.data.list;
          this.broPage=(res.data.data.pagination.total/15)*100;
          this.$refs.noData.isHasData(this.broList)
        }
      }).catch(err=>{
        this.broLoading=false;
      })
    ...
    subBro(){//添加券商
      if(this.addBroMes.name==null||this.addBroMes==''){
        ...
        this.$http.postFormAPI('securities',{
    ...
    editBroSub(){//修改证券
      if(this.editBroMes.name==''||this.editBroMes==null){
        ...
        this.btnLoading=true;
        this.$http.putSomeAPI('securities',{
